...
n_total_wo_continent = len(df_without_south_america)
n_total_continent = len(df_south_america)
...
train_df = df_without_south_america.iloc[:n_train].reset_index(drop=True)
val_df = df_without_south_america.iloc[n_train:n_train+n_val].reset_index(drop=True)
test_df = df_south_america.copy().reset_index(drop=True)
...
def train_phase(num_epochs, phase_name):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(farseg.state_dict(), f"models/best_model_{phase_name}_south_america.pth")
        else:
            patience_counter += 1
...
# save model
torch.save(get_model(farseg).state_dict(), "models/farseg_model_south_america.pth")
...
state_dict = torch.load("models/farseg_model_south_america.pth")
