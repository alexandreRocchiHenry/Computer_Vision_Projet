...
train_dataset = FourBandSegDataset(train_df, apply_augmentation=True)
val_dataset = FourBandSegDataset(val_df, apply_augmentation=False) 
test_dataset = FourBandSegDataset(test_df, apply_augmentation=False) 
...
new_conv.weight.data[:, 3:4, :, :] = model_resnet.conv1.weight.data[:, :3, :, :].mean(dim=1, keepdim=True)
...
def train_phase(num_epochs, phase_name):
    ...
    for epoch in range(num_epochs):
        ...
        val_loss, val_miou, val_acc = evaluate_model(farseg, val_loader, criterion, device=device, num_classes=8)
        scheduler.step(val_loss)

        print(f"Validation Loss: {val_loss:.4f} | Validation mIoU: {val_miou:.4f} | Accuracy: {val_acc:.4f}")
